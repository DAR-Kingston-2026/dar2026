...
public class EmployeService {
     ...
     * Refactorisée pour réduire la complexité cognitive.
     */
    public void calculPerformanceCommercial(String matricule, Long caTraite, Long objectifCa) throws EmployeException {
        
        // 1. Validation des paramètres (Délégation)
        verifierParametresPerformance(matricule, caTraite, objectifCa);
        ...
        // 3. Calcul du palier de performance
        Integer performance = calculerPalierPerformance(employe, caTraite, objectifCa);

        // 4. Bonus performance moyenne
    ...
    /**
     * Vérifie la validité des entrées pour le calcul de performance.
     */
    private void verifierParametresPerformance(String matricule, Long caTraite, Long objectifCa) throws EmployeException {
    ...
    /**
     * Détermine le niveau de performance selon les paliers CA/Objectif.
     */
    private Integer calculerPalierPerformance(Employe employe, Long caTraite, Long objectifCa) {
        int perfActuelle = employe.getPerformance();

        if (caTraite > objectifCa * 1.2) {
            return perfActuelle + 4;
        }
        if (caTraite > objectifCa * 1.05) {
            return perfActuelle + 1;
        }
        if (caTraite >= objectifCa * 0.95) {
            return Math.max(Entreprise.PERFORMANCE_BASE, perfActuelle);
        }
        if (caTraite >= objectifCa * 0.8) {
            return Math.max(Entreprise.PERFORMANCE_BASE, perfActuelle - 2);
        }
